...
for cat_i,(df,cat) in enumerate([ (storm_df,'Stormwater') ,
    ...
    ax.set_yticks([])
    ax.set_xlabel('Rise/Settle Velocity (m/s)')
    ax.axvline(0,color='k',lw=0.8,dashes=[8,16])

axs[1].text(0.0,-0.07,"$Floats$",transform=ax.transAxes,va='top',ha='left')
axs[0].text(1,-0.07,"$Sinks$",transform=ax.transAxes,va='top',ha='right')
